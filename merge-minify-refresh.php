...
class MergeMinifyRefresh
{
	...
	function init()
	{
		...
		*/
		...
		require_once(dirname(__FILE__) . '/WpCliMergeMinifyRefresh.class.php');
		...
		if(is_admin())
		{
			if(current_user_can('administrator'))
			{
				...
				if (($_GET['page'] ?? '') == 'merge-minify-refresh' && ($_GET['action'] ?? '') == 'minifyall')
				{
					do_action('mmr_minify');
		
					add_action( 'admin_notices', function () {
						echo '<div class="notice notice-success">
								<p>All merged assets have been minified.</p>
								</div>'; 
					});
				}
			}
		}
		else if(!defined('MMR_DISABLE') && apply_filters('should_mmr', !is_customize_preview()))
		{
	...
	public function merge_minify_refresh_settings()
	{
		...
		?><div id="merge-minify-refresh">
				<h1>Merge + Minify + Refresh</h1>
				<p>When a CSS or JS file is modified MMR will automatically re-process the files to merge them (minification will then happen offline, each hour). However, when a dependency changes these files may become stale.</p>

				<nav class="nav-tab-wrapper">
					<a id="mmr-tab1" href="#mmr-tab1-content" class="nav-tab nav-tab-active">Processed Files</a>
					<a id="mmr-tab2" href="#mmr-tab2-content" class="nav-tab">Settings</a>
				</nav>
				<div class="tab-content" id="mmr-tab1-content">
					<div id="mmr_processed">
						<a href="#" class="button button-secondary purgeall">Purge All</a>
						<a href="?page=merge-minify-refresh&action=minifyall" class="button button-secondary minifyall">Minify All Now</a>
						<a href="#" class="button button-secondary refresh">Refresh</a>
						<p><i><span id="mmr_refreshed">&nbsp;</span></i></p>

						<div id="mmr_jsprocessed">
							<h4>Javascript</h4>
							<ul class="processed"></ul>
							<p class="noprocess_msg"></p>
						</div>
				...
				<div class="tab-content" id="mmr-tab2-content">
					<form method="post" id="mmr_options" action="options.php">
		<?php
		...
		echo '<p><button type="submit" class="button">SAVE</button></p></form>';
			...
			echo '<p>The behaviour of MMR can also be customised by adding the following constants to <span class="monospace">wp-config.php</span>:<p>';
			echo '<table class="mmr-constants-table">';
			echo '<tr><th>Example</th><th>Description</th></tr>';
			...
			$options = [
				...
				[
					...
					'description' => 'Override server path to store minified files. Must be absolute path.'
				],
				[
				...
				[
					'example' => 'define(\'MMR_DISABLE\', true);',
					'description' => 'Disable merging and minification sitewide.'
				]
			];

			foreach($options as $key => $option)
			{
				echo '<tr><td class="monospace">' . $option['example'] . '</td><td>' . $option['description'] . '</td></tr>';
			}
		?></table>
		  </div><!-- /tab-content -->
		</div><!-- /merge-minify-refresh -->
		<?php
	}
	...
	/**
	 * Log a message to STDOUT if running via WP-CLI, and optionally append it to the given file.
	 * 
	 * @param string $msg
	 * @param string $log_file_path			Optional
	 */
	private function _log_message($msg, $log_file_path = null)
	{
		$log_message = date('c') . ' - ' . $msg;
	...
	private function compress_css($full_path)
	{
		if(is_file($full_path))
		{
			...
			$this->_log_message('Compressing CSS', $full_path . '.log');
			...
			{
				$this->_log_message('Gzipped - ' . $min_path . ".gz", $full_path . '.log');
			}

			$file_size_after = filesize($min_path);
			$this->_log_message("Compression complete - " . $this->human_filesize($file_size_before-$file_size_after) . " saved", $full_path . '.log');
		}
	}
	...
	private function compress_js($full_path)
	{
		if(is_file($full_path))
		{
			...
			if(
				...
				$this->_is_java_version_sufficient($jvoutput[0])
			)
			{
				$this->_log_message("Compressing JS with Closure", $full_path . '.log');
				...
				if(count($output) != 0)
				{
					...
					$this->_log_message("Compression failed", $full_path . '.log');
			...
			else
			{
				...
				$this->_log_message("Compressing with Minify (PHP exec not available or Java 11+ not found)...", $full_path . '.log');
	...
	/**
	 * Confirm the the Java version is 11+
	 * https://github.com/google/closure-compiler/issues/3956
	 * 
	 * @param string $version_output
	 * @return bool
	 */
	private function _is_java_version_sufficient($version_output)
	{
		$sufficient = false;
		if (preg_match('@version "(.+?)"@', $version_output, $matches))
		{
			$tokens = explode('.', $matches[1]);
			$sufficient = $tokens[0] >= 11;
		}
		return $sufficient;
	}
	...
	public function minify_action()
	{
		if($this->cssmin)
		{
			...
			{
				$this->_log_message("Compressing $path...");
